		...
		<div class="container" style="margin-top: 0px" >
			<div class="container"><!--精品菜谱图片-->
		     ...
		     <%
		         ...
		         String strId =(String)request.getParameter("recipeId1");
		         int id=Integer.parseInt(strId);
		         
		         out.print(strId+"  "+id);
		         Recipe recipe=dao.getAllpicture(id);
		         int allId=recipe.getAllID();//当前图片的id
		         ArrayList<Recipe> content=dao.getComment(allId);//获取具体是哪个菜谱哪个人发评论
		         request.getSession().setAttribute("allId", allId); 
		        
		     %>
			
			<div class="col-sm-4 col-xm-4 col-md-4 col-lg-4">
				<img  src=<%=recipe.getPictureName()%> class="img-responsive">
					
			<p>
			    
				<h2><%=recipe.getRecipeName() %></h2> <a href="MyCollectionServelet?allId1=<%=allId%>&accountNumber=<%=accountName%>" ><button class="btn btn-primary">收藏</button></a><br>
				<h4>描述:<%=recipe.getDescription()%></h4>
				<h4>主料:<%=recipe.getMainIngredient()%></h4>
				<h4>辅料:<%=recipe.getFlavour()%></h4>
				<h4>步奏:<%=recipe.getStep()%></h4>
				<h4>评论:<br>
				<% 
				   for(Recipe e:content){
			      %>
			        <table >
			        <tr><p style="color: red"><%=e.getAccountName()%>:<%=e.getContent()%></p></tr>
			  ...
			  <form role="form" action="SaveCommentServelet?rId=<%=recipe.getAllID()%>" method="post">
              <div class="form-group">
              <label for="name">文本框</label>
              <input type="text" class="form-control" rows="3" name="comment" />
              </div>
